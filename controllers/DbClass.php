<?php
// DbClass.php -  Thu Mar 1 09:49:47 CST 2018
//   This is a rewrite/cleanup of the DbClass I have been using.
...
class DbClass 
{
 ...
 // initiator     
//    function DbClass(/* we might pass defaults here */) {                
    public function __construct() {
        ...
        try {
            ...
            // testing with ssl. -- ATTR_EMULATE_PREPARES?
            $this->conn = mysqli_init();
            $this->conn->options(MYSQLI_OPT_SSL_VERIFY_SERVER_CERT, true);
            $result = $this->conn->ssl_set($KEY_FILE, $CERT_FILE, $CA_FILE, NULL, NULL);
            $this->conn->real_connect($DB_SERVER, $DB_USER, $DB_PASSWORD , $DATABASE, $DB_PORT);
  ...
  /////////////////////////////////
  // I assumed that if the input for a varchar was too long, it 
  // would be automatically truncated when the software tried to 
  // add it to the table. This is not the case. It turns out that I 
  // should check this myself in many situtions. This only checks the length
  // varchar strings
   function columnLengthHash($table_title) {
        $desc_table = $this->tableDescription($table_title);
        $descHash = array() ; // for 5.3 compatablility 
    	$matches = [];
         foreach($desc_table as $col_desc) {
            ...
            if(preg_match("/varchar\((\d+)\)/i", $type, $matches)){
            	$descHash[$col_desc['Field']] = $matches[1] ;
            }
        }
  ...
  /////////////////////////////////
  // Get the type character for proper SQL binding.
  ...
  /////////////////////////////////
  // Assuming that the table has an index field called 'id',
  // this will check a row with that id exists.
...
// a primary key called, 'id'
  ...
  // column => values
    function getTableNoParams($sql) {
        ...
        // Older versions of DbClass check to see if $this->has_native_driver
        // is true. From here on out, we won't worry about it. If the server 
        // isn't using php 7.3+ there are a lot more problems than this not working.
    }
...
// e.g. delete a a row 
  ...
  // This is where most of the errors occur. 
    function  prepBindExMultiParams($sql, $typeStr, $paramList){
        try {
           // the mysqli prepare statement returns a function with an namespace.
            $stmt = $this->conn->prepare($sql);
            if (! $stmt) {
                throw new Exception();
            }    
            ...
            // pass the getTableNoParams array of the escaped parameters to the statement --??
            $n = count($paramList);
            for( $i=0; $i < $n; $i++ ) {
                $callbackParams[] = & $paramList[$i];
            }
            
            // The function being passed to call_user_func_array is a member of the $stmt 
            // bind_params. In procedural form, this is mysqli_stmt_bind_param($callbackParams);
